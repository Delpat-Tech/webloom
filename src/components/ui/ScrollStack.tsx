...
//     <motion.div
//       custom={index}
//       className={`scroll-stack-item absolute w-[25rem] h-[30rem] rounded-xl box-border ${itemClassName}`}
//       initial="hidden"
//       animate={getVariant()}
//       whileHover={{
//         scale: 1.05,
...
//     <div
//       ref={containerRef}
//       className={`relative w-full h-[48rem] flex justify-center items-center overflow-visible py-20 ${className}`.trim()}
//       style={{
...
//      >
//       {/* Desktop Cards */}
//       <div className="hidden md:block relative w-[25rem] h-[30rem]">
//         <AnimatePresence>
...
// <div className="block md:hidden w-full space-y-6 px-4 mt-12 mb-6">
//   {childrenArray.map((child, index) => (
//     <motion.div
//       key={index}
...
//       }}
//       className="bg-card border border-border rounded-xl shadow p-4 sm:p-6 max-w-[28rem] mx-auto"
//     >
...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({
  ...
  const getHoverScale = () => {
    switch (screenSize) {
      case 'mobile': return 1.03;
      case 'tablet': return 1.04;
      default: return 1.05;
    }
  };
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className={`relative mb-24 ${containerConfig.width} ${screenSize === 'mobile' ? 'h-[22rem]' : screenSize === 'tablet' ? 'h-[27rem]' : 'h-[30rem]'}`}>
        <AnimatePresence>
          {childrenArray.map((child, index) => (
            <ScrollStackItem
              key={index}
